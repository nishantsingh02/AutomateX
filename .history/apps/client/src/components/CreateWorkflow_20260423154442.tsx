...
export default function CreateWorkflow() {
  ...
  const [showTriggerSheet, setShowTriggerSheet]
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await apiCreateWorkflow({ nodes, edges });
      navigate("/dashboard");
    } catch (err) {
      console.error("Filed to save Workflow", err)
    } finally {
      setLoading(false);
    }
  }
  ...
  <div className="absolute top-0 left-0 right-0 h-20 border-b bg-background/50 backdrop-blur-md z-30 flex items-center justify-between px-8">
    ...
    <Button onClick={handleSave} disabled={loading || nodes.length === 0} className="rounded-full px-6">
