...
import { ArrowLeft, Loader2, ShieldCheck, Lock, User } from "lucide-react";
      ...
      <div className="w-full max-w-[440px] px-6 py-12 relative z-10">
        ...
        <div className="bg-white/70 backdrop-blur-xl border border-white/20 rounded-[32px] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {error && (
              <div className="p-4 rounded-2xl bg-destructive/5 border border-destructive/10 text-destructive text-sm font-medium text-center animate-in fade-in zoom-in duration-300">
                {error}
              </div>
            )}
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 text-center">
            ...
            >
              {isLogin ? (
                <span>Don't have an account? <span className="text-slate-900 font-bold underline-offset-4 hover:underline">Sign up for free</span></span>
              ) : (
                <span>Already have an account? <span className="text-slate-900 font-bold underline-offset-4 hover:underline">Log in here</span></span>
              )}
            </button>
          </div>
        </div>
